// 게임리스트 처리
if (M('[data-game]').selector.length > 0) {
	var gameStr = ''
	for (var i in gameList) {
		gameStr += '<li>'
		+'	<a href="http://game.gaeyou.com/app/#/v/' + gameList[i].idx + '">'
		+'		<div class="pic"><img src="http://game.gaeyou.com/upload/game/thum1/' + gameList[i].thumnail + '"></div>'
		+'		<div class="info">'
		+'			<p class="title">' + gameList[i].title_ko + '</p>'
		+'			<p class="genre">' + gameList[i].genre_ko + '</p>'
		+'			<p class="rating">'
		+'				<span>   깨업: <span>' + gameList[i].gaeup_count + '</span></span>'
		+'				<span> | 댓글: <span>' + gameList[i].reply_count + '</span></span>'
		+'			</p>'
		+'		</div>'
		+'	</a>'
		+'</li>'
	}
	M('[data-game]').html(gameStr)
}
