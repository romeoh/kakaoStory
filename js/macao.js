...
function requestCategory() {
	...
	$.ajax({
		...
		,'success': function(data){
			var data = M.json(data)['body']['categorys'][0]
			M('#category').html(data.category_nm)
			categoryName = data.category_nm
...
function requestThread() {
	...
	sendData.category_idx = categoryIdx
	...
	$.ajax({
		...
		,'type': 'POST'
		,'success': function(data){
			var data = M.json(data)['body']['threads'][0]
			console.log(data)
			M('#thread').html('<b>['+data.thread_title +']:<br></b>'+ data.thread_content.replace(/\n/gi, '<br>'));
			threadIdx = data.thread_idx
			requestTimeline();
		}
	})
}
...
function regist() {
	...
	sendData.category_id = categoryId
	sendData.thread_idx = threadIdx
	sendData.commnad_content = M('#tarea').val().replace(/\n/g, '<br>');
	//console.log(M('#tarea').val())
	bodyData = {
		'body':sendData,
		'head':{}
	}
